...
def action_rescale(action:np.ndarray, action_space, raw_action_range=(-1,1), explore:bool=True, epsilon:float=0.0):
    ...
    action = action * (action_space.high - action_space.low) / 2 + (action_space.high + action_space.low) / 2  #采样后适配物理环境 从[-1,1]变为角度和速度的输出
...
class CarParkingWrapper(Wrapper):
    def __init__(self, env:CarParking, 
        ...
        if 'image' in self.observation_shape:
            w,h,c = self.observation_shape['image']
            self.observation_shape['image'] = (c,w,h)
