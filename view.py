...
for filename in tqdm(sorted(os.listdir(folder_path))):
    ...
    for item in tqdm(os.listdir(path_sring)):
        ...
        if not os.path.exists(file_path):
            continue
        plot_father_path = os.path.join(file_path, "pred.png")
        save_cluster_path = os.path.join(file_path, "pred_cluster.png")
        data_pre_view_path = os.path.join(path_sring, "pred")
        data_cluster_view_path = os.path.join(path_sring, "pred_cluster")
        os.makedirs(data_pre_view_path, exist_ok=True)
        os.makedirs(data_cluster_view_path, exist_ok=True)
        plt.ioff()
        ...
        plt.close()
        img_cluster = Image.open(save_cluster_path)
        plt.imshow(img_cluster)
        plt.savefig(os.path.join(data_cluster_view_path, str(cnt)))
        plt.close() 
        cnt = cnt + 1
